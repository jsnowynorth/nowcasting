...
for(i in 1:66){
  ...
  # proj <- nowcast(bigD = bigD, tolerance = 0, iteration = 10, number = 10,
  #                  timestep = (begin - 1), num = 1, r = err[i,2], nframes = 10, Q = Q0)
  # 
  for(j in 4:58){
    ...
    Q0 <- array(0, dim = dim(bigD[,,begin]))
    proj0 <- nowcast(bigD = bigD, tolerance = 0, iteration = 10, number = 10,
                    timestep = (begin - 1), num = 1, r = err[i,2], nframes = 10, Q = Q0)
    ...
    if(err[i,1] == 0){
      ...
      Q <- (bigD[,,begin] - proj0)/err[i,1]
      # Q <- ifelse(Q < 0.01, 0, Q)
    }
    
    proj <- nowcast(bigD = bigD, tolerance = 0, iteration = 10, number = 10,
                    timestep = begin, num = 10, r = err[i,2], nframes = 1, Q = Q)
